...
def ode_singleneuron_numba(
    M, Pc, Pn, Ca, V, VIP, light, vip_input, gaba_input, p_arr, natural_freq
):
    ...
    return np.array([dM, dPc, dPn, dCa, dV, dVIP]) * natural_freq
...
def ode_all_neurons_numba(
    ...
    for i in range(N):
        ...
        p = params_arr[i]
        natural_freq = natural_freq_array[i]
        dY[i, :] = ode_singleneuron_numba(
            states[i, 0],
            states[i, 1],
            states[i, 2],
            states[i, 3],
...
class SingleCell:
    def __init__(
        self, params: dict, cell_id: int, natural_freq: float, cell_type: str = "core"
    ):
        self.params = params
        self.id = cell_id
...
class CellNetwork:
    def __init__(self, cell_list: list, vip_weight: list, gaba_matrix: np.ndarray):
    ...
    def ode_all_neurons(
        self, t: float, flat_state: np.ndarray, light_func_args
    ) -> np.ndarray:
        ...
        natural_freq_array = np.array(
            [cell.natural_freq for cell in self.cells], dtype=np.float64
        )
