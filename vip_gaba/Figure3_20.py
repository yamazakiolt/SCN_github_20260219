    ...
    for j, vip_shell in enumerate(vip_shell_list):
        ...
        for gaba_sc in [str(round(g / 200, 3)) for g in range(40)]:
            ...
            for L_phase in set(aftereffect_result["L_phase"]):
                ...
                LD_phase = 2 * L_phase
                period = df_period["period"].iloc[0]

                if np.prod(df_entrain["is_sync"]) == 1:
                    x2.append(LD_phase)
                    y2.append(float(gaba_sc))
                    c2.append(cmap(norm(df_period["period"].iloc[0])))
        ...
        if i == 0 and j == 0:
            ax.scatter(x3, y3, c="k", s=10, alpha=0.1, label="Figure3.12")
            ax.scatter(x, y, c="r", s=10, label="after effect")
        else:
            ax.scatter(x3, y3, c="k", s=10, alpha=0.1)
...
fig.legend(loc="upper left", bbox_to_anchor=(1.02, 0.9), fontsize=30, frameon=False)
...
fig.savefig("./results/Figure3_20.png", dpi=300)
