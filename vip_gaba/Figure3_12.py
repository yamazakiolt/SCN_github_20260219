...
def plot_a4_layout():
    ...
    csv_list = sorted(
        [
            g
            for g in os.listdir(f"./VIP_GABA_1_1cell_randint/{vip_condition}")
            if not g.startswith(".")
        ]
    )
    for csv_file in csv_list:
        path = os.path.join("./VIP_GABA_1_1cell_randint/", vip_condition, csv_file)
    ...
    for i, gabasc in enumerate(hist_gabasc):
        ...
        if os.path.exists(path):
            ...
            # print(gabasc, np.mean(x_hist), np.std(x_hist))
            n, bins, patches = plt.hist(x_hist, bins=np.arange(22, 26.1, 0.1).tolist())
            # print(n)
            # print(bins)
            ax_h.hist(
                x_hist,
    ...
    l_g_sc = [0.02, 0.09, 0.09]
    seeds = [250110, 264110, 264125]
    labels_wave = ["(f)", "(g)", "(h)"]
    ...
    for idx, seed in enumerate(seeds):
        ...
        cell_types = ["core"] * 1 + ["shell"] * 1
        vip_weights = [vip_core] * 1 + [vip_shell] * 1
        gaba_mat = np.zeros((cells_num, cells_num))
        for r in range(cells_num):
            for e in range(cells_num):
        ...
        ax_main.scatter(period_data["V0"][1], g_sc, marker="*", c="r", s=400, alpha=0.5)

        if idx == 1:
            labelx = period_data["V0"][1] - 0.5
            labely = g_sc + 0.01
        else:
            labelx = period_data["V0"][1] + 0.1
            labely = g_sc + 0.01
        ax_main.text(
            labelx,
            labely,
    ...
    plt.suptitle("(q)" + "  " + r"$w_C=50, w_S=50, 2 neurons$", fontsize=30, y=0.95)
    fig.legend(
        loc="upper right", fontsize=15, bbox_to_anchor=(0.93, 0.6), handlelength=4
    )
    os.makedirs("./results", exist_ok=True)
    plt.savefig("./results/Figure3_12.png", bbox_inches="tight", dpi=300)
    plt.show()
